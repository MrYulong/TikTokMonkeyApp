...
- (void)viewDidLoad {
    ...
    NSString *plistFile = [[NSBundle mainBundle] pathForResource:@"countryCode" ofType:@"plist"];
...
- (void)saveAction {
    ...
    if (![areaDic isEqualToDictionary:self.areaLists[self.selectedRow]]) {
        ...
        UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:@"提示" message:@"请重新打开TikTok开启冒险🎉" preferredStyle:UIAlertControllerStyleAlert];
        [alertCon addAction:[UIAlertAction actionWithTitle:@"Go! Go! Go!" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            exit(0);
        }]];
